...
/** GET /compras/{id} — misma forma que el POST, mas los campos generados (id, codigo, estado, proveedor_nombre). */
public record CompraDetalleResponse(Integer id, String codigo, Integer proveedorId, String proveedorNombre,
                                     Integer usuarioId, String numeroFacturaProv, LocalDate fecha, String formaPago,
                                     String estado, String observaciones, @Monto BigDecimal total,
    ...
    public static CompraDetalleResponse de(CompraVista vista) {
        ...
        return new CompraDetalleResponse(c.getId(), c.getCodigo(), c.getProveedorId(), vista.proveedorNombre(),
                c.getUsuarioId(), c.getNumeroFacturaProv(), c.getFecha(), c.getFormaPago(), c.getEstado(),
                c.getObservaciones(), c.getTotal(), vista.items().stream().map(CompraDetalleItemResponse::de).toList());
    }
}
