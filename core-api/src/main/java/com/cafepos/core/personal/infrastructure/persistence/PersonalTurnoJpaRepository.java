...
interface PersonalTurnoJpaRepository extends TenantAwareRepository<Turno, Integer> {
    ...
    @Query(value = "SELECT t.id AS id, t.empleado_id AS empleado_id, e.nombre AS empleado_nombre, "
            + "t.fecha AS fecha, t.hora_inicio AS hora_inicio, t.hora_fin AS hora_fin, "
            + "t.horas_trabajadas AS horas_trabajadas, t.observaciones AS observaciones "
